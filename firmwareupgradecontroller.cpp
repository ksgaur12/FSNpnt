...
void FirmwareUpgradeController::flash(QString localfile)
{
    localfile = localfile.remove(0, 7);
    connect(this, &FirmwareUpgradeController::downloadFinished, this, &FirmwareUpgradeController::_firmwareDownloadFinished);

    if (_bootloaderFound) {
        emit downloadFinished(localfile, localfile);
//        _firmwareDownloadFinished(localfile, localfile);
    } else {
        // We haven't found the bootloader yet. Need to wait until then to flash
        _startFlashWhenBootloaderFound = true;
//        _startFlashWhenBootloaderFoundFirmwareIdentity = firmwareId;
